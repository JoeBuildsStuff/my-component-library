...
<div className="w-full flex justify-end">
    <OpenInV0Button url="https://my-component-library-woad.vercel.app/r/supabase-auth.json" />
</div>

<Tabs groupId='package-manager' persist items={['npm', 'pnpm', 'yarn', 'bun']} label='Install the supabase auth system'>

    ```bash tab="npm"
    npx shadcn@latest add https://my-component-library-woad.vercel.app/r/supabase-auth.json
    ```

    ```bash tab="pnpm"
    pnpm dlx shadcn@latest add https://my-component-library-woad.vercel.app/r/supabase-auth.json
    ```

    ```bash tab="yarn"
    yarn dlx shadcn@latest add https://my-component-library-woad.vercel.app/r/supabase-auth.json
    ```

    ```bash tab="bun"
    bunx --bun shadcn@latest add https://my-component-library-woad.vercel.app/r/supabase-auth.json
    ```
</Tabs>
...
- Check your email templates in Supabase dashboard
...
- Ensure server/client are using same domain

### Rate Limiting

The system includes built-in rate limiting. Users will see friendly error messages when limits are exceeded:

```tsx
// Rate limit handling in auth actions
if (error.code === 'over_email_send_rate_limit') {
  redirect(`/signin?error=rate_limit&message=${encodeURIComponent(error.message)}`)
}
```
